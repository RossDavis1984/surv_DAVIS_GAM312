...
//set up find object function,  and set up line trace, establishing hit result local variable 
{
			...
			if (Stamina > 5.0f)
			{
				...
				if (HitResource)
				{
					...
					if (HitResource->totalResource > resourceValue)
					{
						...
						//updates number of materials collected and passes it when called
	...
	{
		isBuilding = false;
		//updates number of objects built with+1 and calls it
...
void APlayerChar::SpawnBuilding(int buildingID, bool& isSuccess)
{
	if (!isBuilding)
	{
		if (BuildingArray[buildingID] >= 1)
		{
			...
			FVector StartLocation = PlayerCamComp->GetComponentLocation();
			FVector Directon = PlayerCamComp->GetForwardVector() * 400.0f;
			FVector EndLocation = StartLocation + Directon;
